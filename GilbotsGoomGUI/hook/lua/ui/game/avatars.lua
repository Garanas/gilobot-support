...
--# If the Interface option is set...
if options.gui_idle_engineer_avatars ~= 0 then

--# This is hooked to add middle-click 
--# handler to select all idle SCUs on screen
...
--# Scu manager adds 2 new "idle engineer" classes 
--# for SCUs that have full upgrade paths
--# Hook this to add the 2 extra idle engineer classes
    ...
    group.Update = function(self, unitData)
        local function CreateUnitEntry(techLevel, userUnits, icontexture)
            ...
            if techLevel == 'C' or techLevel == 'E' then
                entry.techIcon = Bitmap(entry, UIUtil.UIFile('scumanager/tech-'..techLevel..'_bmp.dds', true))
            else
        ...
        if table.getsize(tempSCUs) > 0 then
            for i, unit in tempSCUs do
                if SCUManager.IsPathComplete(unit, 'Combat') then
                    table.insert(engineers[7], unit)
                elseif SCUManager.IsPathComplete(unit, 'Engineer') then
...
--# Hooked this to display the SCU manager upgrade 
--# buttons when there is an idle SCU without a 
--# full path applied.
    ...
    if controls.idleEngineers then
        ...
        if table.getsize(subCommanders) > 0 then
            for i, unit in subCommanders do
                --# If this SCU hasn't finished an
                --# upgrade path...
                if not SCUManager.GetSCUType(unit) then
                    --# We can show controls
                    --# to set idle SCUs on their paths
                    show = true
                    break
                end
            end
            --# If we found an idle SCU earlier and the buttons
            --# aren't already showing... 
            if show and not ExtraButtonsMenu.HasButton('Combat') then
                --# Add the buttons.
                ExtraButtonsMenu.AddButton('Combat')
                ExtraButtonsMenu.AddButton('Engineer')
            end
        end
    end
    --# If the buttons should not be showing...
    if not show then
        --# Remove both buttons
        ExtraButtonsMenu.RemoveButton('Combat')
        ExtraButtonsMenu.RemoveButton('Engineer')
    end
end
    
    
end--(of SCUmanager UI option)
